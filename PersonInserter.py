...
class CandidatePatch:
    ...
    target_face_rgb: list[np.ndarray]  # group 中每个个体的面部rgb
    source_face_rgb: np.ndarray        # individual的面部rgb
...
def _cluster_rows(
    ...
    """按 face y 聚类成行：相邻 face y 的 gap > max(20, 0.04*H) 视为新行。"""
    if not persons:
        return []
    sorted_p = sorted(persons, key=lambda p: p["face"]["center"][1])
    threshold = max(20.0, 0.04 * image_height)
    rows: list[list[dict[str, Any]]] = [[sorted_p[0]]]
    for p in sorted_p[1:]:
        prev_y = rows[-1][-1]["face"]["center"][1]
        if p["face"]["center"][1] - prev_y > threshold:
            rows.append([p])
        else:
            rows[-1].append(p)
    for row in rows:
...
def _rank_rows_by_face_h(
    rows: list[list[dict[str, Any]]],
) -> dict[int, int]:
    """给每个 row_index 排一个深度等级：行内平均 face_h 越大 = 越靠前（rank 越大）。"""
    row_avg: list[tuple[int, float]] = []
    for r_idx, row in enumerate(rows):
        if not row:
            row_avg.append((r_idx, 0.0))
            continue
        avg = sum(p["face"]["bbox"][3] for p in row) / len(row)
        row_avg.append((r_idx, avg))
    # 升序：face_h 小的 = 远 = 排在前面（rank 小）
...
def _compute_patch(
    ...
    target_bbox_list: list[list[int]],
    ...
    hair = max(0, hair)
    hair = min(0.5, hair)
    indiv_face_rgb = individual_rgb[int(round((1 - hair) * source_bbox[1] + hair * source_bbox[3])):source_bbox[3], source_bbox[0]:source_bbox[2]]
    target_face_rgb = [group_rgb[int(round((1 - hair) * bbox[1] + hair * bbox[3])):bbox[3], bbox[0]:bbox[2]] for bbox in target_bbox_list]
...
def find_insertion_patches(
    ...
    group_bbox_list = [[person["face"]["bbox"][0], person["face"]["bbox"][1], person["face"]["bbox"][0] + person["face"]["bbox"][2], person["face"]["bbox"][1] + person["face"]["bbox"][3]] for person in persons]
    if not persons:
        return []
    ...
    indiv_bbox = [individual["face"]["bbox"][0], individual["face"]["bbox"][1], individual["face"]["bbox"][0] + individual["face"]["bbox"][2], individual["face"]["bbox"][1] + individual["face"]["bbox"][3]]
    ...
    row_rank = _rank_rows_by_face_h(rows)
